...
function kazi(options,callback){
	...
	KAZI.options=_.extend(
		{	
			...
			"dataFiles" : path.join(__dirname,'..','job_data'),
	...
	mkdirp.sync(KAZI.options.dataFiles);
...
kazi.prototype={
		...
		if(_.has(body,'jobs')){
			...
			async.eachLimit(jobs,1,function(job,next){
				...
				db.run(query,function(rows){	
					...
					var file=path.join(KAZI.options.dataFiles , md5(job.id) +'.json');
					del.sync(file);
	...
	requestJob: function(body,callback){
		...
		db.select(query,function(rows){
			...
			if(rows.length>0){
				...
				//save as json file...
				var file=path.join(KAZI.options.dataFiles , md5(rows[0].id) +'.json');
	...
	queueJobs: function(body,callback){
		...
		async.eachLimit(jobs,1,function(job,next){
			...
			if((job=KAZI.formatJob(job))){
				...
				if(_.has(job,'data') &&  job.data){
					// json file has four space indenting now 
					jsonFile.writeFileSync(file, job.data);	
				}
	...
	jobSQL: function(job,callback){
		...
		var fields=_.merge(
						{   "id":job.id,
							"name":job.name,
							// "data": {"file": md5(job.id)}
						},
						job.meta
					)

		//pick fields
		fields= _.pick(fields, ['id','name','created','updated','next_run','delay','version','reschedule_after','reschedule_at','priority','status','ttl'] );
